...
<div class="layui-layout layui-layout-admin">
    <div class="layui-header">
        <div class="layui-logo" style="font-weight: bold;font-size: 19px">通讯录管理系统</div>
        ...
        <div class="search_style" style="padding-top: 10px;padding-left: 15px;">
            <div class="searchTable">
                ...
                <div class="layui-inline">
                    ...
                    <input type="hidden" class="layui-input" name="searchColumn" id="searchColumn" autocomplete="off" value="contact_name">
                </div>
...
<script>
    ...
    layui.use('table', function () {
        ...
        table.render({
            elem: '#myData',
            id: 'myTable',
            url: 'ContactServlet?action=list',
        ...
        table.on('tool(myTable)', function (obj) { //注：tool 是工具条事件名，myTable 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'del') { //删除
                // layer.confirm('删除' + data.contactName + "?", function (index) {
                $.ajax({
                    type: "GET",
                    dataType: "text",
                    url: "ContactServlet?action=delete&id=" + data.id,
                    success: function () {
                        console.log("已删除！");
                    }
                });
                ...
                // layer.close(index);
                //parent.location.reload();//刷新父级页面
                layer.msg(data.contactName + ' 删除成功！');
                // });
                ...
                layer.open({
                    ...
                    fixed: false, //不固定
                    maxmin: true,
                    content: 'ContactServlet?action=editPre&id=' + data.id
                });
                ...
                layer.open({
                    ...
                    fixed: false, //不固定
                    maxmin: true,
                    content: 'ContactServlet?action=get&id=' + data.id
                });
                obj.update({});//同步更新缓存对应的值
        ...
        let $ = layui.$, active = {
            reload: function () {
                let url = 'ContactServlet?action=list&keyword=' + $('#search_keyword').val() + '&searchColumn=' + $('#searchColumn').val();
                //执行重载
